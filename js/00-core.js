...
function _audit(extra={}){
  const u=DB.obj('settings').managerName||'النظام';
  return {createdAt:_now(),updatedAt:_now(),createdBy:u,updatedBy:u,isDeleted:false,deletedAt:null,deletedBy:null,version:1,...extra};
}
...
// ══════════════════════════════════════════
const DB={
  _fb:['patients','appointments','inventory','invoices','invoice_items','services','leads',
       'doctors','staff','expenses','branches','campaigns','packages','sessions',
       'waitlist','rooms','equipment','suppliers','purchases','purchase_items',
       'visits','inventory_transactions','audit_log'],
  get(k){try{return JSON.parse(localStorage.getItem('ha_'+k))||[];}catch{return [];}},
  obj(k){try{return JSON.parse(localStorage.getItem('ha_'+k))||{};}catch{return {};}},
  ...
  upd(k,id,d){
    const a=DB.get(k),i=a.findIndex(x=>x.id==id);
    if(i>=0){
      const u=DB.obj('settings').managerName||'النظام';
  ...
  del(k,id){DB.set(k,DB.get(k).filter(x=>x.id!=id));if(DB._fb.includes(k))fbDel(k,id);}
};

// ══════════════════════════════════════════
// 🌱 SEED DATA — Schema v2 (UUID + relational)
// ══════════════════════════════════════════
