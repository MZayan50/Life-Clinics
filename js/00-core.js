...
// 🏥 CLINIC CORE — v3.0
// ══════════════════════════════════════════════════════════════════
// الترتيب الصحيح لهذا الملف:
//   1. Logo + clinicLogoHTML
//   2. gv() helper — مشترك لكل الملفات
//   3. UUID + Audit
//   4. DB Layer (push/upd/del/softDel) — لا يُعاد تعريفه في أي ملف آخر
//   5. showScreen موحدة — يُضاف إليها مباشرة بدل patch chain
//   6. Seed Data
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════
// 🛠️ SHARED HELPER — gv() متاحة لكل الملفات
// كانت محصورة في 02-patients.js — نقلناها هنا
// ══════════════════════════════════════════
...
function _audit(extra = {}){
  const u = (function(){ try{ return JSON.parse(localStorage.getItem('ha_settings') || '{}').managerName || 'النظام'; }catch{ return 'النظام'; } })();
...
// ══════════════════════════════════════════
// 🗃️ DB LAYER — localStorage + Firestore sync
// ══════════════════════════════════════════
// ⚠️ هذا الكائن لا يُعاد تعريفه أو استبداله في أي ملف آخر.
// 06-finance.js كان يعيد تعريف DB.push — تم حذف ذلك من هناك.
// ══════════════════════════════════════════
const DB = {
  // Collections التي تتزامن مع Firestore تلقائياً
  // أضفنا: photos, installments, cashlog
  _fb: [
    ...
    'photos', 'installments', 'cashlog'   // ← مضافة (كانت مفقودة)
  ],
  ...
  // push: يضيف UUID + audit + يزامن Firestore
  push(k, o){
    const a = DB.get(k);
  ...
  // upd: يحدّث السجل + يزيد version + يزامن Firestore
  upd(k, id, d){
    const a = DB.get(k);
  ...
  // softDel: soft delete بدون حذف فعلي (للـ audit trail)
  softDel(k, id){
    const u = (function(){ try{ return JSON.parse(localStorage.getItem('ha_settings')||'{}').managerName||'النظام'; }catch{ return 'النظام'; } })();
    DB.upd(k, id, { isDeleted: true, deletedAt: _now(), deletedBy: u });
  },

  // del: حذف فعلي من localStorage + Firestore
  del(k, id){
...
// ══════════════════════════════════════════
// 🔗 LOOKUP HELPERS — جلب الاسم من ID بأمان
// تحل مشكلة الـ Denormalization: بدل حفظ الاسم
// في كل record، نجلبه runtime من المصدر
// ══════════════════════════════════════════
...
function _patByName(name){
  // fallback: البحث بالاسم إذا كان patientId غير متوفر (بيانات قديمة)
  return DB.get('patients').find(p => p.name === name);
}
function _invsByPat(patientId, patientName){
  // يدعم البحث بالـ ID أولاً، ثم بالاسم كـ fallback للبيانات القديمة
  return DB.get('invoices').filter(i =>
    (patientId && i.patientId == patientId) || i.patient === patientName
  );
}


// ══════════════════════════════════════════
// 🧭 showScreen — دالة موحدة بدل 5 patches
// ══════════════════════════════════════════
// الشاشات وعناوينها
const TITLES = {
...
function showScreen(id){
  // إخفاء كل الشاشات وتفعيل المطلوبة
  document.querySelectorAll('.screen').forEach(s => s.classList.remove('active'));
  document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active'));
  const el = document.getElementById('screen-' + id);
  if (el) el.classList.add('active');

  // تحديث العنوان والتاريخ
  ...
  // ── Render calls لكل شاشة (موحدة هنا بدل 5 patches مبعثرة) ──
  ...
  if (id === 'treasury')       renderTreasury();   // ← كانت مفقودة
