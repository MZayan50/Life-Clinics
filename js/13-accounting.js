...
function validateSystemIntegrity(){
  ...
  // 5. فواتير بإجمالي صفر أو سالب — باستثناء فواتير الجلسات المغطاة بباقة نشطة
  // (دي بتتسجل عمدًا total=0/paid=0 لتجنب ازدواج الإيراد، لأن قيمتها اتحصّلت
  // بالفعل وقت شراء الباقة نفسها — راجع الفاتورة اللي بتتعمل في 07-clinical.js
  // مع pkgId + method:'باقة'). الصفر هنا مقصود ومش خطأ.
  (DB.get('invoices')||[]).forEach(i=>{
    if(i.pkgId || i.method==='باقة') return;
    if(!(i.total > 0)){
