...
function renderAppts(){
  const appts=DB.get('appointments').filter(a=>(!_af||a.patient.includes(_af))&&(!_as||a.status===_as)&&(!_ad||a.date===_ad));
  const tb=document.getElementById('appts-tbody');if(!tb)return;
  tb.innerHTML=appts.map(a=>`<tr><td style="font-weight:600">${a.patient}</td><td style="font-size:12px">${a.date}</td><td style="font-weight:700;color:var(--teal)">${a.time}${a.endTime?` <span style="color:var(--text-muted);font-weight:400;font-size:11px">- ${a.endTime}</span>`:''}</td><td>${a.service}</td><td>${a.doctor||'—'}</td><td style="font-size:12px">${a.branch||'—'}</td><td><span class="ast ${ASC[a.status]||'sd'}">${a.status}</span></td><td style="display:flex;gap:5px;white-space:nowrap;"><button class="btn btn-ghost btn-xs" onclick="openApptModal('${a.id}')">✏️</button><button class="btn btn-ghost btn-xs" onclick="updAppt('${a.id}','مكتمل')">✅</button><button class="btn btn-teal btn-xs" onclick="sendApptWA('${a.id}')" title="إرسال الموعد عبر واتساب">💬</button><button class="btn btn-danger btn-xs" onclick="delAppt('${a.id}')">🗑</button></td></tr>`).join('');
  renderTodayAppts();
}
function updAppt(id,st){DB.upd('appointments',id,{status:st});renderAppts();renderReception();renderDoctorView();renderWaitlist();showToast('success','✅ تم تحديث حالة الموعد');}
function delAppt(id){if(confirm('حذف الموعد؟')){DB.del('appointments',id);renderAppts();buildCal();renderReception();renderDoctorView();renderWaitlist();}}
...
function saveAppt(){
  ...
  clearApptConflictBox();
  const data={patId:pid,patient:pat?.name||pid,service:svcName,type:gv('am-type'),doctor,date,time,endTime,duration,room,equipment,branch:gv('am-branch'),notes:gv('am-notes')};
  if(id){DB.upd('appointments',id,data);showToast('success','✅ تم تحديث الموعد');}
  else{DB.push('appointments',{...data,status:'مؤكد'});showToast('success','📅 تم حجز الموعد بنجاح',`${pat?.name} - ${date} ${time}`);}
  closeModal('appt-modal');renderAppts();buildCal();renderTodayAppts();renderReception();renderDoctorView();renderWaitlist();
...
// Checks doctor / room / equipment / working-hours / leave conflicts for a proposed appointment slot.
...
// Suggests up to `count` nearest free time slots (15-minute steps) for the same doctor/room/equipment/date.
...
function renderTodayAppts(){
  ...
  el.innerHTML=ta.slice(0,5).map((a,i)=>`<div class="appt-item"><div class="appt-time">${a.time}</div><div class="appt-ava" style="background:${AVA[i%AVA.length]}">👩</div><div class="appt-info"><p>${a.patient}</p><span>${a.service}</span></div><span class="ast ${ASC[a.status]||'sd'}">${a.status}</span></div>`).join('')||'<div style="text-align:center;color:var(--text-muted);padding:20px;font-size:13px;">لا توجد مواعيد اليوم</div>';
  txt('kpi-appt',ta.length);txt('badge-appts',ta.length);
}
...
function calSel(el,ds){
  ...
  if(el2)el2.innerHTML=da.map(a=>`<div class="appt-item"><div class="appt-time">${a.time}</div><div class="appt-ava" style="background:linear-gradient(135deg,#8B5CF6,#3B82F6);font-size:13px">👩</div><div class="appt-info"><p style="font-size:12.5px">${a.patient}</p><span style="font-size:11px">${a.service}</span></div><span class="ast ${ASC[a.status]||'sd'}" style="font-size:10px">${a.status}</span></div>`).join('')||'<div style="text-align:center;color:var(--text-muted);padding:20px;font-size:12.5px">لا توجد مواعيد</div>';
}
