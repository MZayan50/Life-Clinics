...
function showReport(type){
  ...
  if(report.byService){
    ...
    svcs.forEach(([k,v])=>{
      ...
      extra += `<div style="margin-bottom:8px;"><div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:4px;"><span>${escapeHtml(k)}</span><span style="font-weight:700;color:var(--teal)">${v.toLocaleString()} ج</span></div><div style="height:6px;background:var(--glass);border-radius:3px;overflow:hidden;"><div style="height:100%;background:var(--teal);width:${Math.min(pct,100)}%;border-radius:3px;"></div></div></div>`;
    });
    extra += '</div>';
  }
  if(report.byMethod){
    ...
    Object.entries(report.byMethod).forEach(([k,v])=>{
      extra += `<div style="display:flex;justify-content:space-between;padding:8px 12px;background:var(--glass);border-radius:8px;margin-bottom:6px;font-size:13px;"><span>${escapeHtml(k)}</span><span style="font-weight:700;color:var(--gold-light)">${v.toLocaleString()} ج</span></div>`;
    });
    extra += '</div>';
  }
  if(report.bySource){
    ...
    Object.entries(report.bySource).forEach(([k,v])=>{
      extra += `<div style="display:flex;justify-content:space-between;padding:8px 12px;background:var(--glass);border-radius:8px;margin-bottom:6px;font-size:13px;"><span>${escapeHtml(k)}</span><span style="font-weight:700;color:var(--purple)">${v} عميل</span></div>`;
    });
    extra += '</div>';
  }
  if(report.lowStock && report.lowStock.length){
    ...
    report.lowStock.forEach(i=>{
      extra += `<div style="display:flex;justify-content:space-between;padding:8px 12px;background:rgba(245,158,11,.07);border:1px solid rgba(245,158,11,.2);border-radius:8px;margin-bottom:6px;font-size:13px;"><span>${escapeHtml(i.name)}</span><span style="color:var(--amber)">متبقي: ${i.qty}</span></div>`;
    });
    extra += '</div>';
  }
  if(report.doctorsTable){
    ...
    report.doctorsTable.forEach((d,i)=>{
      ...
      extra += `<tr style="border-bottom:1px solid var(--glass-border);"><td style="padding:9px 8px;font-weight:700;">${i===0?'🥇 ':i===1?'🥈 ':i===2?'🥉 ':''}${escapeHtml(d.name)}</td><td style="padding:9px 8px;color:var(--text-muted);font-size:12px;">${escapeHtml(d.specialty)}</td><td style="padding:9px 8px;text-align:center;color:var(--purple);font-weight:700;">${d.patients}</td><td style="padding:9px 8px;text-align:center;color:var(--teal);font-weight:700;">${d.sessions}</td><td style="padding:9px 8px;text-align:left;"><div style="color:${colors[i%5]};font-weight:800;">${d.revenue.toLocaleString()} ج</div><div style="height:4px;background:var(--glass);border-radius:2px;margin-top:4px;overflow:hidden;"><div style="height:100%;background:${colors[i%5]};width:${pct}%;"></div></div></td></tr>`;
    });
  ...
  if(report.branchesTable){
    ...
    report.branchesTable.forEach((b,i)=>{
      ...
      extra += `<tr style="border-bottom:1px solid var(--glass-border);"><td style="padding:9px 8px;font-weight:700;">${i===0?'🏆 ':'🏢 '}${escapeHtml(b.name)}</td><td style="padding:9px 8px;text-align:center;color:var(--purple);font-weight:700;">${b.patients}</td><td style="padding:9px 8px;text-align:center;color:var(--teal);font-weight:700;">${b.appointments}</td><td style="padding:9px 8px;text-align:left;"><div style="color:${colors[i%4]};font-weight:800;">${b.revenue.toLocaleString()} ج</div><div style="height:4px;background:var(--glass);border-radius:2px;margin-top:4px;overflow:hidden;"><div style="height:100%;background:${colors[i%4]};width:${pct}%;"></div></div></td></tr>`;
    });
    extra += '</tbody></table></div></div>';
  }
  if(report.byChannel){
    ...
    Object.entries(report.byChannel).forEach(([ch,data],i)=>{
      ...
      extra += `<div style="background:var(--glass);border-radius:var(--radius-sm);padding:12px;margin-bottom:8px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;"><span style="font-weight:700;">${escapeHtml(ch)}</span><span style="color:${colors[i%5]};font-weight:700;">${data.budget.toLocaleString()} ج</span></div><div style="display:flex;gap:12px;font-size:12px;color:var(--text-muted)"><span>ليدز: <b style="color:var(--text-primary)">${data.leads}</b></span><span>محوّل: <b style="color:var(--emerald)">${data.converted}</b></span><span>تحويل: <b style="color:${conv>=30?'var(--teal)':'var(--amber)'}">${conv}%</b></span></div></div>`;
    });
    extra += '</div>';
  }
...
function exportReportPDF(type, modalEl){
  ...
  if(report.byService){
    ...
    svcs.forEach(([k,v])=>{ const pct=Math.round(v/total*100); bodyHtml+=`<div style="margin-bottom:7px;"><div style="display:flex;justify-content:space-between;font-size:12.5px;margin-bottom:3px;"><span>${escapeHtml(k)}</span><span style="font-weight:700;color:#0D9488">${v.toLocaleString()} ج (${pct}%)</span></div><div style="height:5px;background:#e5e7eb;border-radius:3px;overflow:hidden;"><div style="height:100%;background:#0D9488;width:${pct}%;"></div></div></div>`; });
  }
  if(report.byMethod){
    bodyHtml += `<h3 style="font-size:13px;font-weight:700;color:#333;margin:16px 0 8px;">طرق الدفع</h3><table style="width:100%;border-collapse:collapse;font-size:12.5px;"><thead><tr style="background:#f8f4ef;"><th style="padding:7px;text-align:right;border-bottom:2px solid #C4A882;">الطريقة</th><th style="padding:7px;text-align:left;border-bottom:2px solid #C4A882;">المبلغ</th></tr></thead><tbody>${Object.entries(report.byMethod).map(([k,v])=>`<tr style="border-bottom:1px solid #f0f0f0;"><td style="padding:7px;">${escapeHtml(k)}</td><td style="padding:7px;font-weight:700;color:#047857;">${v.toLocaleString()} ج</td></tr>`).join('')}</tbody></table>`;
  }
  if(report.bySource){
    bodyHtml += `<h3 style="font-size:13px;font-weight:700;color:#333;margin:16px 0 8px;">مصادر العملاء</h3><table style="width:100%;border-collapse:collapse;font-size:12.5px;"><thead><tr style="background:#f8f4ef;"><th style="padding:7px;text-align:right;border-bottom:2px solid #C4A882;">المصدر</th><th style="padding:7px;text-align:center;border-bottom:2px solid #C4A882;">العدد</th></tr></thead><tbody>${Object.entries(report.bySource).map(([k,v])=>`<tr style="border-bottom:1px solid #f0f0f0;"><td style="padding:7px;">${escapeHtml(k)}</td><td style="padding:7px;text-align:center;font-weight:700;">${v}</td></tr>`).join('')}</tbody></table>`;
  }
  if(report.doctorsTable){
    bodyHtml += `<h3 style="font-size:13px;font-weight:700;color:#333;margin:16px 0 8px;">أداء الأطباء</h3><table style="width:100%;border-collapse:collapse;font-size:12.5px;"><thead><tr style="background:#f8f4ef;"><th style="padding:7px;text-align:right;border-bottom:2px solid #C4A882;">#</th><th style="padding:7px;text-align:right;border-bottom:2px solid #C4A882;">الطبيب</th><th style="padding:7px;text-align:center;border-bottom:2px solid #C4A882;">التخصص</th><th style="padding:7px;text-align:center;border-bottom:2px solid #C4A882;">العملاء</th><th style="padding:7px;text-align:center;border-bottom:2px solid #C4A882;">الجلسات</th><th style="padding:7px;text-align:left;border-bottom:2px solid #C4A882;">الإيرادات</th></tr></thead><tbody>${report.doctorsTable.map((d,i)=>`<tr style="border-bottom:1px solid #f0f0f0;"><td style="padding:7px;font-size:11px;color:#999;">${i+1}</td><td style="padding:7px;font-weight:700;">${escapeHtml(d.name)}</td><td style="padding:7px;text-align:center;font-size:11px;color:#666;">${escapeHtml(d.specialty)}</td><td style="padding:7px;text-align:center;">${d.patients}</td><td style="padding:7px;text-align:center;">${d.sessions}</td><td style="padding:7px;font-weight:700;color:#047857;">${d.revenue.toLocaleString()} ج</td></tr>`).join('')}</tbody></table>`;
  }
  if(report.branchesTable){
    bodyHtml += `<h3 style="font-size:13px;font-weight:700;color:#333;margin:16px 0 8px;">أداء الفروع</h3><table style="width:100%;border-collapse:collapse;font-size:12.5px;"><thead><tr style="background:#f8f4ef;"><th style="padding:7px;text-align:right;border-bottom:2px solid #C4A882;">الفرع</th><th style="padding:7px;text-align:center;border-bottom:2px solid #C4A882;">العملاء</th><th style="padding:7px;text-align:center;border-bottom:2px solid #C4A882;">المواعيد</th><th style="padding:7px;text-align:left;border-bottom:2px solid #C4A882;">الإيرادات</th></tr></thead><tbody>${report.branchesTable.map(b=>`<tr style="border-bottom:1px solid #f0f0f0;"><td style="padding:7px;font-weight:700;">${escapeHtml(b.name)}</td><td style="padding:7px;text-align:center;">${b.patients}</td><td style="padding:7px;text-align:center;">${b.appointments}</td><td style="padding:7px;font-weight:700;color:#047857;">${b.revenue.toLocaleString()} ج</td></tr>`).join('')}</tbody></table>`;
  }
  if(report.byChannel){
    bodyHtml += `<h3 style="font-size:13px;font-weight:700;color:#333;margin:16px 0 8px;">القنوات التسويقية</h3><table style="width:100%;border-collapse:collapse;font-size:12.5px;"><thead><tr style="background:#f8f4ef;"><th style="padding:7px;text-align:right;border-bottom:2px solid #C4A882;">القناة</th><th style="padding:7px;text-align:center;border-bottom:2px solid #C4A882;">الميزانية</th><th style="padding:7px;text-align:center;border-bottom:2px solid #C4A882;">الليدز</th><th style="padding:7px;text-align:center;border-bottom:2px solid #C4A882;">المحوّل</th><th style="padding:7px;text-align:left;border-bottom:2px solid #C4A882;">نسبة التحويل</th></tr></thead><tbody>${Object.entries(report.byChannel).map(([k,d])=>`<tr style="border-bottom:1px solid #f0f0f0;"><td style="padding:7px;font-weight:700;">${escapeHtml(k)}</td><td style="padding:7px;text-align:center;">${d.budget.toLocaleString()} ج</td><td style="padding:7px;text-align:center;">${d.leads}</td><td style="padding:7px;text-align:center;color:#047857;font-weight:700;">${d.converted}</td><td style="padding:7px;font-weight:700;">${d.leads?Math.round(d.converted/d.leads*100):0}%</td></tr>`).join('')}</tbody></table>`;
  }
  if(report.lowStock && report.lowStock.length){
    bodyHtml += `<h3 style="font-size:13px;font-weight:700;color:#B45309;margin:16px 0 8px;">⚠️ منتجات تحتاج إعادة طلب</h3><table style="width:100%;border-collapse:collapse;font-size:12.5px;"><thead><tr style="background:#fef9c3;"><th style="padding:7px;text-align:right;border-bottom:2px solid #B45309;">المنتج</th><th style="padding:7px;text-align:center;border-bottom:2px solid #B45309;">الكمية المتبقية</th><th style="padding:7px;text-align:center;border-bottom:2px solid #B45309;">حد إعادة الطلب</th></tr></thead><tbody>${report.lowStock.map(i=>`<tr style="border-bottom:1px solid #f0f0f0;"><td style="padding:7px;font-weight:700;">${escapeHtml(i.name)}</td><td style="padding:7px;text-align:center;color:#dc2626;font-weight:700;">${i.qty}</td><td style="padding:7px;text-align:center;">${i.reorder}</td></tr>`).join('')}</tbody></table>`;
  }

  const html = `<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>${escapeHtml(report.title)}</title>
...
<div class="header"><div style="display:flex;align-items:center;gap:10px;"><div>${clinicLogoHTML(38)}</div><div><div class="logo">${escapeHtml(clinicName)}</div><div class="logo-sub">📞 ${escapeHtml(clinicPhone)}</div></div></div>
<div class="rpt-meta"><span>تقرير رسمي</span><strong>${escapeHtml(report.title)}</strong><div style="margin-top:3px">📅 ${now}</div></div></div>
${bodyHtml}
