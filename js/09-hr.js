...
function docRevenue(name){
  ...
  return DB.get('invoices').filter(i=>svcNames.includes(i.service)).reduce((s,i)=>s+(i.paid||0),0);
}
function renderDocs(){
  ...
  grid.innerHTML=docs.map((d,i)=>{
    const rev=docRevenue(d.name),comm=Math.round(rev*(d.commission||0)/100),today=docApptsToday(d.name),total=docApptsTotal(d.name);
    return `<div class="bcard">
      <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:12px;">
        <div style="display:flex;gap:10px;align-items:center;">
          <div class="tdava" style="background:${AVA[i%AVA.length]}">👨‍⚕️</div>
          <div><div style="font-size:15px;font-weight:800;">${d.name}</div><div style="font-size:11.5px;color:var(--text-muted);margin-top:2px;">${d.specialty||'—'}</div></div>
        </div>
        <span class="ast ${d.status==='نشط'?'sc':'sd'}">${d.status}</span>
      </div>
      <div class="g2c" style="gap:8px;margin-bottom:11px;">
        <div style="background:var(--glass);border-radius:8px;padding:9px;text-align:center;"><div style="font-size:18px;font-weight:800;color:var(--teal)">${today}</div><div style="font-size:11px;color:var(--text-muted)">مواعيد اليوم</div></div>
        <div style="background:var(--glass);border-radius:8px;padding:9px;text-align:center;"><div style="font-size:18px;font-weight:800;color:var(--gold-light)">${rev.toLocaleString()}</div><div style="font-size:11px;color:var(--text-muted)">إيرادات (ج)</div></div>
      </div>
      <div style="font-size:12px;color:var(--text-muted);margin-bottom:10px;">📍 ${d.branch||'—'} · ${total} جلسة إجمالي · عمولة ${d.commission||0}% (${comm.toLocaleString()} ج)</div>
...
function saveDoc(){
  const name=gv('doc-name').trim();if(!name){showToast('warning','⚠️ اسم الطبيب مطلوب');return;}
  const id=gv('doc-id');
  const data={name,specialty:gv('doc-specialty'),branch:gv('doc-branch'),phone:gv('doc-phone'),email:gv('doc-email'),commission:parseFloat(gv('doc-commission'))||0,status:gv('doc-status'),workStart:gv('doc-workstart')||'09:00',workEnd:gv('doc-workend')||'17:00',offDay:gv('doc-offday'),leaveDates:gv('doc-leaves')};
  ...
  closeModal('doctor-modal');renderDocs();
}
function delDoc(id){
  const d=DB.get('doctors').find(x=>x.id===id);
  if(confirm(`حذف ${d?.name||'الطبيب'}؟`)){DB.del('doctors',id);showToast('info','🗑 تم الحذف');renderDocs();}
}
function fillDocDropdowns(){
  ...
  ['am-doc','sv-doc'].forEach(did=>{
    ...
    const cur=sel.value;
    sel.innerHTML=docs.map(d=>`<option value="${d.name}">${d.name}</option>`).join('')||'<option value="">-- لا يوجد أطباء --</option>';
...
function fillSvcDropdowns(){
  ...
  const cur=sel.value;
  sel.innerHTML=svcs.map(s=>`<option value="${s.name}">${s.name} (${s.duration||60} د)</option>`).join('')||'<option value="">-- لا توجد خدمات --</option>';
...
function fillApptRoomDropdown(svcName){
  ...
  sel.innerHTML='<option value="">— تلقائي حسب الخدمة —</option>'+DB.get('rooms').map(r=>`<option>${r.name}</option>`).join('');
...
function renderStaff(){
  ...
  grid.innerHTML=staff.map((s,i)=>`<div class="bcard">
      <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:12px;">
        <div style="display:flex;gap:10px;align-items:center;">
          <div class="tdava" style="background:${AVA[i%AVA.length]}">👩‍💼</div>
          <div><div style="font-size:15px;font-weight:800;">${s.name}</div><span class="tag ${roleColors[s.role]||'tg-teal'}" style="font-size:11px;margin-top:3px;display:inline-block;">${s.role}</span></div>
        </div>
    ...
    </div>`).join('')||'<div class="card" style="text-align:center;padding:40px;color:var(--text-muted);">لا يوجد موظفون مطابقون</div>';
}
...
function saveStaff(){
  const name=gv('st-name').trim();if(!name){showToast('warning','⚠️ اسم الموظف مطلوب');return;}
  const id=gv('st-id');
  const data={name,role:gv('st-role'),branch:gv('st-branch'),phone:gv('st-phone'),email:gv('st-email'),salary:parseFloat(gv('st-salary'))||0,status:gv('st-status')};
  if(id){DB.upd('staff',id,data);showToast('success',`✅ تم تحديث بيانات ${name}`);}
  else{DB.push('staff',data);showToast('success',`✅ تم إضافة ${name}`);}
  closeModal('staff-modal');renderStaff();
}
function delStaff(id){
  const s=DB.get('staff').find(x=>x.id===id);
  if(confirm(`حذف ${s?.name||'الموظف'}؟`)){DB.del('staff',id);showToast('info','🗑 تم الحذف');renderStaff();}
}
function payStaffSalary(id){
  ...
  DB.push('expenses',{name:`راتب: ${s.name}`,type:'رواتب',amount:s.salary||0,branch:s.branch,date:new Date().toISOString().split('T')[0],notes:''});
...
function renderSvcs(){
  ...
  tb.innerHTML=DB.get('services').map(s=>{const pr=s.price-s.cost,pt=s.price?Math.round(pr/s.price*100):0;return `<tr><td style="font-weight:600">${s.name}</td><td><span class="tag tg-teal">${s.cat}</span></td><td style="color:var(--gold-light);font-weight:700">${s.price} ج</td><td>${s.duration} د</td><td>${s.cost} ج</td><td style="color:var(--emerald)">${pr} ج (${pt}%)</td><td style="font-size:12px">${s.doctor}</td><td style="white-space:nowrap"><button class="btn btn-ghost btn-xs" onclick="openSvcModal('${s.id}')">✏️</button> <button class="btn btn-danger btn-xs" onclick="delSvc('${s.id}')">🗑</button></td></tr>`;}).join('');
}
function delSvc(id){if(confirm('حذف الخدمة؟')){DB.del('services',id);renderSvcs();}}
function openSvcModal(id){
  ...
  if(roomSel){roomSel.innerHTML='<option value="">— بدون تحديد —</option>'+DB.get('rooms').map(r=>`<option>${r.name}</option>`).join('');roomSel.value=s?(s.room||''):'';}
  const equipSel=document.getElementById('sv-equip');
  if(equipSel){equipSel.innerHTML='<option value="">— بدون تحديد —</option>'+DB.get('equipment').map(e=>`<option>${e.name}</option>`).join('');equipSel.value=s?(s.equipment||''):'';}
  openModal('service-modal');
}
function saveSvc(){
  const name=gv('sv-name').trim();if(!name){showToast('warning','⚠️ اسم الخدمة مطلوب');return;}
  const id=gv('sv-id');
  const data={name,cat:gv('sv-cat'),doctor:gv('sv-doc'),price:parseFloat(gv('sv-price'))||0,cost:parseFloat(gv('sv-cost'))||0,duration:parseInt(gv('sv-dur'))||60,room:gv('sv-room'),equipment:gv('sv-equip')};
  if(id){DB.upd('services',id,data);showToast('success',`✅ تم تحديث "${name}"`);}
  else{DB.push('services',data);showToast('success',`✅ تم إضافة "${name}"`);}
  closeModal('service-modal');renderSvcs();
}
