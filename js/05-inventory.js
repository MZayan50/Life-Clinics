...
// ══════════════════════════════════════════
// 💰 قيمة المخزون بسعر التكلفة (موحَّدة)
// ✅ FIX (مراجعة مالية): كانت قيمة المخزون في الميزانية وتقرير المخزون
// تُحسب بسعر البيع (item.price) بدل سعر التكلفة، ما يُضخّم الأصول والربحية
// الظاهرة بفارق هامش الربح. هذه الدالة الموحَّدة تُستخدم الآن من الشاشتين معاً
...
function openSupplierModal(id){
  ...
  ['id','name','cat','phone','email','whatsapp','contact','terms','status','notes'].forEach(f => {
...
function saveSupplier(){
  ...
  const data = { name, cat:gv('sup-cat'), phone:gv('sup-phone'), email:gv('sup-email'),
                 whatsapp:gv('sup-whatsapp'), contact:gv('sup-contact'), terms:gv('sup-terms'),
